package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1407:int = 5;
      ...
      protected var var_1384:Boolean = false;
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_449 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_16.contentLoaderInfo.url + "\"");
               var_449 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_371 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_371 + " for file \"" + var_16.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
         ...
         if(var_449 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_449 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_449 == LibraryLoader.STATE_READY)
         {
            var_1384 = true;
            prepareLibrary();
            removeEventListeners();
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_940 > 0)
            {
               var_16.close();
               var_16.unload();
               addRequestCounterToUrlRequest(var_326,const_1407 - var_940);
