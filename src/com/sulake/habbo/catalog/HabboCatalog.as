package com.sulake.habbo.catalog
{
   ...
   public class HabboCatalog extends Component implements IHabboCatalog
   {
      ...
      private var var_2027:GiftWrappingConfiguration;
      ...
      private function onCreditBalance(param1:IMessageEvent) : void
      {
         ...
         var_153.credits = _loc3_.balance;
         updatePurse();
         if(!var_2026 && var_509 != null)
         {
            var_509.playSound(HabboSoundTypesEnum.const_1007);
         }
         var_2026 = false;
      }
      ...
      private function onRoomSessionEvent(param1:RoomSessionEvent) : void
      {
         switch(param1.type)
         {
            case RoomSessionEvent.const_91:
               var_1270 = param1.session.isPrivateRoom;
               if(var_83 != null)
               {
      ...
      private function noCreditsConfirmDialogEventProcessor(param1:IConfirmDialog, param2:Event) : void
      {
         ...
         {
            if(false && "true" == var_145.getKey("client.credits.embed.enabled"))
            {
               ExternalInterface.call("FlashExternalInterface.openHabblet","credits");
            }
            else
            {
               _loc3_ = var_145.getKey("link.format.credits");
               if(_loc3_ != "")
               {
      ...
      private function onGiftWrappingConfiguration(param1:GiftWrappingConfigurationEvent) : void
      {
         ...
         var_2027 = new GiftWrappingConfiguration(param1);
      }
      ...
      private function toggleCatalog(param1:Boolean = false) : void
      {
         ...
         {
            var_196.events.dispatchEvent(new HabboToolbarShowMenuEvent(HabboToolbarShowMenuEvent.const_46,HabboToolbarIconEnum.CATALOGUE,_mainContainer));
         }
         if(false)
         {
      ...
      private function onClubGiftInfo(param1:ClubGiftInfoEvent) : void
      {
         ...
         var_1031.setInfo(_loc2_.daysUntilNextGift,_loc2_.giftsAvailable,_loc2_.offers,_loc2_.giftData);
      }
      ...
      private function onSubscriptionInfo(param1:IMessageEvent) : void
      {
         ...
         var_153.clubDays = Math.max(0,_loc2_.daysToPeriodEnd);
         var_153.clubPeriods = Math.max(0,_loc2_.periodsSubscribedAhead);
         updatePurse();
      ...
      private function createClubGiftController() : void
      {
         if(var_1031 == null)
         {
            var_1031 = new ClubGiftController(this);
         }
      }
      ...
      {
         var_196.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_115,HabboToolbarIconEnum.CATALOGUE));
      }
      ...
      private function noPixelsConfirmDialogEventProcessor(param1:IConfirmDialog, param2:Event) : void
      {
         ...
         if(param2.type == WindowEvent.const_158)
         {
            _loc3_ = var_145.getKey("link.format.pixels");
            if(_loc3_ != "")
            {
               _windowManager.alert("${catalog.alert.external.link.title}","${catalog.alert.external.link.desc}",0,onExternalLink);
               HabboWebTools.navigateToURL(_loc3_,"habboMain");
      ...
      private function updatePurse() : void
      {
         ...
         _localization.registerParameter("catalog.purse.creditbalance","balance",String(var_153.credits));
         _localization.registerParameter("catalog.purse.pixelbalance","balance",String(var_153.pixels));
         _localization.registerParameter("catalog.purse.clubdays","days",String(var_153.clubDays));
         _localization.registerParameter("catalog.purse.clubdays","months",String(var_153.clubPeriods));
         if(false)
         {
            return;
         }
      ...
      private function onPixelsBalance(param1:IMessageEvent) : void
      {
         ...
         var_153.pixels = _loc2_.amount;
         updatePurse();
         if(!var_2028 && var_509 != null)
         {
            var_509.playSound(HabboSoundTypesEnum.const_946);
         }
         var_2028 = false;
      }
